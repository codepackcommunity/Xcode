...
const getUserRole = (email) => {
  ...
  if (emailLower === process.env.NEXT_KM_SUPER) {
    return 'superadmin';
  } else if (emailLower === process.env.NEXT_KM_ADMIN) {
    return 'manager';
  } else if (emailLower === process.env.NEXT_KM_MANAGER) {
    return 'manager';
  } else {
    return 'user';
  }
...
const handleFirestoreError = (error, context = '') => {
  if (error.code === 'permission-denied') {
    ...
    if (!currentUser) {
      // User is not authenticated - will be handled by the auth flow
      return 'Authentication error. Please try again.';
    }
    return 'Permission denied. Please contact administrator.';
  }
  return `Error ${context ? `in ${context}: ` : ''}${error.message || 'Please try again.'}`;
};
      ...
      if (isLogin) {
        ...
        if (!approved) {
          ...
          try {
            ...
            // Silent fail on sign out error
          }
          setIsLoading(false);
          return;
        }

        // Update last login timestamp if user data exists
        ...
        try {
          ...
          const errorMessage = handleFirestoreError(firestoreError, 'storing user data');
          // If we can't store user data, delete the auth user to clean up
          try {
            await user.delete();
          } catch (deleteError) {
        ...
        if (!autoApprove) {
          try {
            ...
            // Silent fail on approval request creation - user is still created
          }
        }
